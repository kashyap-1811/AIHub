...
namespace AIHub.API.Services
{
    ...
    {
        public string ServiceName => "ChatGPT";
        ...
        public async Task<string> SendMessageAsync(string message, string apiKey, string? conversationId = null)
        {
            try
            {
                ...
                {
                    // Log/return full body so you can debug
                    return $"Error: {response.StatusCode} - {responseContent}";
                }

                // Parse result (OpenRouter uses OpenAI-like response shape)
        ...
        public async Task<bool> ValidateApiKeyAsync(string apiKey)
        {
            try
            {
                ...
                Console.WriteLine($"OpenRouter /key response: {response.StatusCode} - {responseBody}");

                // 200 means valid; other codes indicate invalid / no permission / throttled
